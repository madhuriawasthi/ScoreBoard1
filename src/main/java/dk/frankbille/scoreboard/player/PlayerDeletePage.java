...
public class PlayerDeletePage extends SecureBasePage {
	...
	private void initialize(IModel<Player> playerModel) {
		...
		Form<Player> playerForm = new Form<Player>("playerForm", playerModel) {
			...
			protected void onSubmit() {
				try {
				...
				}catch(Exception e1){          //Edited by Madhuri
					System.out.println("Player could not be delete");
					User user = userModel.getObject();
					if (user != null) {
						scoreBoardService.updateUser(user);
					...
					getRequestCycle().setResponsePage(PlayerListPageException.class);
		...
		userModel = new LoadableDetachableModel<User>() {
			...
			@Override
			protected User load() {
                User user = scoreBoardService.getUserForPlayer(PlayerDeletePage.this.playerModel.getObject());
                return user;
			}
		};
